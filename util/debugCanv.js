function vdebug4(device, size, texture, dispCode=/*wgsl*/`vec4(info.rgba)`){
    ...
    code: /*wgsl*/`
      ...
      @fragment
        fn fragmentMain(
          ...
          let info = textureSampleLevel(tex, filteringSampler, vec2f(
            pos.x/${size[0]},
            1-pos.y/${size[1]}), 0);
          return ${dispCode};
        }
  ...
  const samBgl = c_bgl(device, "debug BGL", [{r:"s"},{r:"t"}]);
  const samBG = c_bg(samBgl, "debug BG", [
  ...
  const fill = (copy=true)=>{
    ...
    if(copy){
      const arrayBuffer = buffer.mapAsync(GPUMapMode.READ,0,4*size[0]*size[1]).then(()=>{
        buf = buffer.getMappedRange(0,4*size[0]*size[1]);
